...
class _ProfilePopupMenuState extends State<ProfilePopupMenu> {
  ...
  void initState() {
    super.initState();
    _loadProfilePhoto(); // Fetch the profile picture when the widget initializes
  }
  ...
  Future<void> _loadProfilePhoto() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('http://localhost:5000/api/profile/photo'), // Ensure this matches your backend
        headers: {
      ...
      if (response.statusCode == 200) {
        setState(() {
          _profilePhotoBytes = response.bodyBytes; // Set the fetched image bytes
        });
      } else {
      ...
      // No need to set anything if the photo cannot be loaded
    }
  }
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<Menu>(
      icon: _profilePhotoBytes != null
          ? CircleAvatar(
              radius: 20,
              backgroundImage: MemoryImage(_profilePhotoBytes!), // Display the profile picture
            )
          : Container(
